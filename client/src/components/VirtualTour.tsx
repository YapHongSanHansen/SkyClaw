 ...
 *
 * UI layout matches Google Street View exactly:
 *  - Full bright panorama fills the entire screen (no dark overlay)
 *  - Top-left: white info card with location name + floor
 *  - Bottom-left: mini-map thumbnail with orange pegman dot
 *  - Floating navigation arrows in the scene (click to walk)
 *  - Top-right: fullscreen + close controls
 *  - Click anywhere to walk toward that direction
 *  - Drag to look around freely
 */
import { useEffect, useRef, useState, useCallback } from "react";
import * as THREE from "three";
import { X, Maximize2, Minimize2, ArrowUp, ChevronLeft } from "lucide-react";
...
export interface Viewpoint {
  ...
  mapX: number;
  mapY: number;
  /** Connected viewpoint IDs */
  connects: number[];
}

export const VIEWPOINTS: Viewpoint[] = [
  { id: 1, label: "Upper Landing",    sublabel: "2nd Floor · Cafe",  floor: "upper",  connects: [2],      url: "https://d2xsxph8kpxj0f.cloudfront.net/310519663328986460/MFw4EaEo6cHsQkkfMQCXGv/cafe-pano-1_1c4e8fa8.jpeg", pos: { x:  8, z: -8 }, mapX: 75, mapY: 18 },
  { id: 2, label: "Staircase Mid",    sublabel: "Staircase · Cafe",  floor: "upper",  connects: [1, 3],   url: "https://d2xsxph8kpxj0f.cloudfront.net/310519663328986460/MFw4EaEo6cHsQkkfMQCXGv/cafe-pano-2_ac2cb743.jpeg", pos: { x:  5, z: -4 }, mapX: 65, mapY: 38 },
  { id: 3, label: "Seating Area",     sublabel: "Ground Floor · Cafe", floor: "ground", connects: [2, 4, 5], url: "https://d2xsxph8kpxj0f.cloudfront.net/310519663328986460/MFw4EaEo6cHsQkkfMQCXGv/cafe-pano-3_e9de660f.jpeg", pos: { x:  3, z:  0 }, mapX: 55, mapY: 58 },
  { id: 4, label: "Window Side",      sublabel: "Ground Floor · Cafe", floor: "ground", connects: [3, 5],   url: "https://d2xsxph8kpxj0f.cloudfront.net/310519663328986460/MFw4EaEo6cHsQkkfMQCXGv/cafe-pano-4_bb2a7c9e.jpeg", pos: { x: -4, z:  1 }, mapX: 20, mapY: 53 },
  { id: 5, label: "Main Hall",        sublabel: "Ground Floor · Cafe", floor: "ground", connects: [3, 4, 6], url: "https://d2xsxph8kpxj0f.cloudfront.net/310519663328986460/MFw4EaEo6cHsQkkfMQCXGv/cafe-pano-5_0e240249.jpeg", pos: { x:  0, z:  4 }, mapX: 40, mapY: 73 },
  { id: 6, label: "Bar Counter",      sublabel: "Ground Floor · Cafe", floor: "ground", connects: [5, 7],   url: "https://d2xsxph8kpxj0f.cloudfront.net/310519663328986460/MFw4EaEo6cHsQkkfMQCXGv/cafe-pano-6_0178eefa.jpeg", pos: { x: -2, z:  7 }, mapX: 30, mapY: 86 },
  { id: 7, label: "Spiral Staircase", sublabel: "Ground Floor · Cafe", floor: "ground", connects: [6],      url: "https://d2xsxph8kpxj0f.cloudfront.net/310519663328986460/MFw4EaEo6cHsQkkfMQCXGv/cafe-pano-7_d5f67861.jpeg", pos: { x:  2, z:  9 }, mapX: 50, mapY: 90 },
];
...
function findBestViewpoint(clickDir: THREE.Vector3, currentId: number): Viewpoint {
  const current = VIEWPOINTS.find((v) => v.id === currentId)!;
  const clickBearing = Math.atan2(clickDir.x, clickDir.z);
  ...
  for (const vp of VIEWPOINTS) {
    ...
    const vpBearing = Math.atan2(dx, dz);
...
export default function VirtualTour({ initialId = 5, onClose }: VirtualTourProps) {
  ...
  useEffect(() => {
    ...
    const makeSphereMesh = (opacity: number) => {
      ...
      geo.scale(-1, 1, 1);
      return new THREE.Mesh(geo, new THREE.MeshBasicMaterial({ color: 0x888888, transparent: true, opacity }));
    };
    ...
    const animate = () => {
      ...
      if (isPanning.current) {
        ...
        if (Math.abs(targetLon.current - lon.current) < 0.05 && Math.abs(targetLat.current - lat.current) < 0.05) {
      ...
      if (isFadingRef.current) {
        fadeProgress.current = Math.min(fadeProgress.current + 0.05, 1);
      ...
      camera.lookAt(lonLatToTarget(lon.current, lat.current));
  ...
  useEffect(() => {
    setIsLoading(true);
    new THREE.TextureLoader().load(VIEWPOINTS.find(v => v.id === initialId)!.url, (tex) => {
  ...
  const navigateTo = useCallback((id: number, clickDir?: THREE.Vector3) => {
    ...
    if (clickDir) {
      targetLon.current = THREE.MathUtils.radToDeg(Math.atan2(clickDir.x, clickDir.z));
      targetLat.current = Math.max(-25, Math.min(25, THREE.MathUtils.radToDeg(Math.asin(Math.max(-1, Math.min(1, clickDir.y))))));
  ...
  const handleNavigationClick = useCallback((clientX: number, clientY: number) => {
    ...
    if (best.id !== currentIdRef.current) {
      ...
      targetLon.current = THREE.MathUtils.radToDeg(Math.atan2(dir.x, dir.z));
      targetLat.current = Math.max(-60, Math.min(60, THREE.MathUtils.radToDeg(Math.asin(Math.max(-1, Math.min(1, dir.y))))));
      isPanning.current = true;
    }
  }, [navigateTo]);
  ...
  const onMouseUp = (e: React.MouseEvent) => {
    ...
    if (dragDistance.current < 6) {
      ...
      if (timeSinceLast < 400) {
        ...
        setTimeout(() => {
          if (Date.now() - lastClickTime.current >= 380) handleNavigationClick(e.clientX, e.clientY);
        }, 390);
      }
    }
  };
  ...
  const onTouchEnd = (e: React.TouchEvent) => {
    ...
    if (dragDistance.current < 12) {
      ...
      if (now - lastTapTime.current < 400) {
        ...
        setTimeout(() => {
          if (Date.now() - lastTapTime.current >= 380) handleNavigationClick(t.clientX, t.clientY);
        }, 390);
      }
      lastTapTime.current = now;
    }
  ...
  // ── Mini-map (bottom-left, like Google Maps) ─────────────────────────────────
  const MiniMap = () => (
    <div
      className="absolute bottom-4 left-4 z-30 rounded-lg overflow-hidden shadow-lg"
      style={{ width: 160, height: 120, border: "2px solid rgba(255,255,255,0.3)", background: "#e8e0d8" }}
    >
        ...
        {([[1,2],[2,3],[3,4],[3,5],[4,5],[5,6],[6,7]] as [number,number][]).map(([a, b]) => {
          ...
          return (
            <line key={`${a}-${b}`}
              ...
              stroke="#aaa" strokeWidth="0.8" strokeDasharray="2,2"
            />
          );
        })}
        ...
        {VIEWPOINTS.map((vp) => {
          ...
          return (
            <g key={vp.id} style={{ cursor: "pointer" }} onClick={() => navigateTo(vp.id)}>
              ...
              {isCurrent ? (
                <>
                  {/* Pegman-style orange dot for current position */}
  ...
  return (
    <div className={`relative overflow-hidden ${isFullscreen ? "fixed inset-0 z-50" : "w-full h-full rounded-lg"}`}
      style={{ background: "#000" }}>

      {/* ── Three.js canvas — full bright, no overlay ── */}
      <div
        ref={mountRef}
      ...
      {!isLoading && (
        <div
          ...
          style={{ background: "rgba(32,33,36,0.95)", minWidth: 200, maxWidth: 280, border: "none" }}
        >
          {/* Back button row */}
      ...
      {/* ── Navigation hint toast (center-bottom) ── */}
      {navHint && (
        <div
          className="absolute bottom-20 left-1/2 -translate-x-1/2 z-40 rounded-full px-4 py-1.5 shadow-lg"
      ...
      {!isLoading && (
        ...
        >
          <span style={{ fontFamily: "sans-serif", fontSize: 10, color: "rgba(255,255,255,0.5)", letterSpacing: "0.05em" }}>
            Drag to look · Click anywhere to walk · {VIEWPOINTS.length} locations
          </span>
        </div>
      )}
